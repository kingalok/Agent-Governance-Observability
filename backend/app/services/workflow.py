...
DEFAULT_ALLOWED_ACTIONS_BY_RISK: dict[str, set[str]] = {
    ...
    RiskTier.HIGH.value: {"triage_and_route", "summarize", "classify", "finalize_external_delivery", "modify_customer_record"},
}
...
def _persist_run_state(
    ...
    *,
    ...
    run.risk_label = RiskTier(risk_value) if risk_value else None
...
def build_poc_workflow(
    ...
    def policy_check_node(state: WorkflowState) -> WorkflowState:
        ...
        allowed_for_agent = requested_tool in agent.allowed_tools
        ...
        state["policy_allowed"] = allowed_for_agent and allowed_by_policy
    ...
    def approval_gate_node(state: WorkflowState) -> WorkflowState:
        ...
        if state.get("approval_decision") == ApprovalStatus.REJECTED.value:
            state["status"] = WorkflowRunStatus.STOPPED.value
            state["stop_requested"] = True
            state["stop_reason"] = "Human approval rejected the request."
    ...
    def tool_execution_node(state: WorkflowState) -> WorkflowState:
        ...
        requested_tool = state.get("requested_tool", "")
        if requested_tool not in agent.allowed_tools:
            state["status"] = WorkflowRunStatus.POLICY_VIOLATION.value
            state["execution_status"] = "blocked"
            state["policy_allowed"] = False
            state["policy_violation_reason"] = f"Execution attempted with disallowed tool '{requested_tool}'."
            state["final_message"] = state["policy_violation_reason"]
            _append_log(state, "tool_execution_blocked", state["final_message"], LogLevel.ERROR.value)
            _persist_run_state(
                db,
                state["run_id"],
                "tool_execution_node",
        ...
        mock_result = {
            "send_email": {
                "status": "simulated",
                "recipient": state["request_payload"].get("destination", "external-requestor@example.com"),
            },
            "create_ticket": {"status": "simulated", "ticket_id": f"TKT-{state['run_id']:04d}"},
            "update_record": {"status": "simulated", "record_id": state["request_payload"].get("record_id", "REC-001")},
        }.get(requested_tool, {"status": "simulated", "detail": "No-op tool"})

        state["execution_status"] = "completed"
        state["execution_result"] = {"tool": requested_tool, **mock_result}
        state["final_message"] = f"Tool '{requested_tool}' executed in demo mode."
        _append_log(
            state,
            "tool_executed",
            state["final_message"],
